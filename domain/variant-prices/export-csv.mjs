...
export function mapVariantPriceNodeToExportRow(variant) {
  ...
  return {
    ...
    option1_name: variant?.option1_name ?? option1Name,
    option1_value: variant?.option1_value ?? option1Value,
    option2_name: variant?.option2_name ?? option2Name,
    option2_value: variant?.option2_value ?? option2Value,
    option3_name: variant?.option3_name ?? option3Name,
    option3_value: variant?.option3_value ?? option3Value,
    price: variant?.price ?? "",
    product_handle: variant?.product_handle ?? product?.handle ?? "",
...
export function serializeVariantPriceCsvRow(row) {
  return PRODUCT_VARIANT_PRICES_EXPORT_HEADERS.map((header) => csvCell(row[header])).join(",");
}

export function createVariantPriceExportCsvBuilder({ signingKey }) {
  ...
  function ensureHeader() {
    if (!wroteHeader) {
      wroteHeader = true;
      return appendRecord(PRODUCT_VARIANT_PRICES_EXPORT_HEADERS.join(","));
    }
  ...
  return {
    appendVariants(variants) {
      ...
      for (const variant of variants) {
        csvChunk += appendRecord(serializeVariantPriceCsvRow(mapVariantPriceNodeToExportRow(variant)));
        exportedRowCount += 1;
      }
