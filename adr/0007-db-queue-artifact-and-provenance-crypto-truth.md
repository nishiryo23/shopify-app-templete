...
テンプレートは webhook 処理と system sweep のために durable queue と private artifact 層を持つ。webhook inbox とは別に worker が `Job` / `Artifact` を扱う。

`Session` / `Shop` / `WebhookInbox` に加え、`Job` / `JobAttempt` / `Artifact` と、offline token 暗号化から独立した optional provenance signing（`domain/provenance`）の扱いを固定する。
...
- artifact visibility はテンプレでは `private` のみ許可し、public URL 前提の保存は採用しない。
...
## Consequences
- ドメイン機能は共通 queue と artifact adapter を前提に ticket で積み上げられる（`adr/0021-harness-template-scope.md`）。
...
- ドメインを追加する場合、artifact retention は kind ごとに `domain/retention/policy.mjs` と observability contract で再定義する。
